...
if "indexer" not in st.session_state:
    with st.spinner("Loading vector index..."):
        ...
        # Mock elements — replace with real EA data in Phase 5
        mock_elements = [
...
with st.sidebar:
    ...
    # Privacy Layer toggle
    privacy_enabled = st.toggle(
        "🔒 Privacy Layer",
        value=middleware.enabled,
        help="When enabled, element names and identifiers are obfuscated before being sent to the LLM."
    )
    ...
    st.metric("Element count", middleware.obfuscator.element_count)
    ...
    with st.chat_message("assistant"):
        with st.status("Thinking...", expanded=True) as status:
            ...
            if middleware.enabled:
                ...
                obfuscated = middleware.translator.obfuscate_text(prompt)
                preview = obfuscated[:120] + "..." if len(obfuscated) > 120 else obfuscated
                st.write(f"📤 Sending to LLM: `{preview}`")
            else:
                st.write("⚠️ Privacy layer disabled — sending raw input to LLM...")
